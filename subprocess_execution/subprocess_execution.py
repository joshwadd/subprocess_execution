...
def shared_memory_context(
...
) -> ContextManager[ManagedSharedMemory]:
    ...
    This context manager is designed to alter the behavior of the multiprocessing resource
    tracker with regard to shared memory management to resolve (https://bugs.python.org/issue38119).

    Args:
        size: Size of the shared memory block in bytes.
        name: Name of the shared memory block. If None, a unqiue name will be generated.
        create: If true, a new shared memory block is created.
            If false, an attempt is made to access and exsisting block.
        deallocate: If true, the shared memory object will be deallocated on exit of the context.

    Note: If deallocate=False the user will be resonsible for dellocating the memory. This could persist
          after the python iterpreter is killed.
    ...
    def patch_resource_tracker() -> None:
        """Patch multiprocessing.resource_tracker so ManagedSharedMemory won't be tracked.
        ...
        def fix_register(name: Sized, rtype: Any) -> None:
            if rtype == "shared_memory" and is_managed_shm(str(name)):
                return
            return resource_tracker._resource_tracker.register(self, name, rtype)  # type: ignore # noqa: F821

        resource_tracker.register = fix_register

        def fix_unregister(name: Sized, rtype: Any) -> None:
            if rtype == "shared_memory" and is_managed_shm(str(name)):
                return
            return resource_tracker._register(self, name, rtype)  # type: ignore # noqa: F821
...
with shared_memory_context(name='{in_shm_name}', create=False, deallocate=False) as input_shm:
...
with shared_memory_context(name="{out_shm_name}", create=True, size=len(serialized_result) + 1, deallocate=False) as shm_result:
        ...
        def wrapper(*args: tuple[Any, ...], **kwargs: dict[str, Any]) -> Optional[Any]:
            ...
            with shared_memory_context(
                ...
                if proc.poll() is not None and proc.returncode != 0:
                    ...
                    clear_subprocess_pipes(proc.stdout, proc.stderr)
                    raise Exception(
                        f"Error: Subprocess terminated unexpectedly with return code {proc.returncode}."
                    )
